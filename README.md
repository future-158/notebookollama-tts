...
I heavily borrowed from  [llama-recipe](https://github.com/meta-llama/llama-recipes/tree/main/recipes/quickstart/NotebookLlama)
(borrowed is understatement here. basically copied.)

modified:
- use ollama instead of hf 
- use tts model with voice cloning feature.
    so, podcast host and guest can use different reference audio
    (currently host use jordan peterosn and guest use david attenbourgh)
...
https://github.com/user-attachments/assets/c3ae3766-ffa4-467b-8a8f-48e338592885
...
*simply run demo-e2e.py file in vscode interactively.*

before that,
you should install ollama and pull models first and install neccessary python packages.
i used [justfile](https://github.com/casey/just) here, (it's like Makefile but better). 



for download ollama models,
you can simply run 
```bash
just download
```
...
i recommend using new conda environment using 

```bash
conda create -y --prefix $PWD/venv python=3.11 pip ipykernel
conda activate $PWD/venv
```

for install python pacakges.
```bash
just install
```
